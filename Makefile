...
.PHONY: help check lint tests test tests-mcp tests-install mcp-env docker-build-renderer docker-ensure-renderer docker-test mcp-build mcp-init mcp-check mcp-smoke mcp-down mcp-stdio render-diagram render-examples render-gallery render-gallery-local clean
...
help:
	...
	@printf "  make mcp-build        Prepare the locked environment and renderer image\n"
	@printf "  make mcp-smoke        Run MCP plus Mermaid/PlantUML Docker smokes\n"
	@printf "  make mcp-down         Remove renderer containers for PROJECT only\n"
	@printf "  make mcp-stdio        Serve the MCP through stdio\n"
...
tests-mcp: mcp-env
	@$(CLI) --project "$(PROJECT)" mcp-smoke >/dev/null
...
tests-install: mcp-env
	...
	@set -euo pipefail; consumer="$$(mktemp -d)"; \
	.tmp/install-wheel/bin/diavisuals --project "$$consumer" init >/dev/null; \
	.tmp/install-wheel/bin/diavisuals --project "$$consumer" factory-check >/dev/null; \
	.tmp/install-wheel/bin/diavisuals --project "$$consumer" mcp-smoke >/dev/null
	...
	.tmp/install-sdist/bin/diavisuals --project "$$consumer" factory-check >/dev/null; \
	.tmp/install-sdist/bin/diavisuals --project "$$consumer" mcp-smoke >/dev/null
...
mcp-build: docker-ensure-renderer mcp-env
	@$(CLI) --project "$(PROJECT)" install-check --command "$(CURDIR)/$(CLI)" >/dev/null
	@$(CLI) --project "$(PROJECT)" factory-check >/dev/null

mcp-init: mcp-env
	@$(CLI) --project "$(PROJECT)" init >/dev/null

mcp-check: mcp-env check
	@$(CLI) --project "$(PROJECT)" lifecycle-check --command "$(CURDIR)/$(CLI)" >/dev/null

docker-test: docker-ensure-renderer
	@$(CLI) --project "$(PROJECT)" render-diagram-text --text 'graph TD; A[Smoke] --> B[SVG]' --output ".cache/diavisuals/smoke/mermaid.svg" --format svg --no-data >/dev/null
	@printf '%s\n' '@startuml' 'Alice -> Bob : Smoke' '@enduml' | $(CLI) --project "$(PROJECT)" render-diagram-text --output ".cache/diavisuals/smoke/plantuml.pdf" --format pdf --no-data >/dev/null
