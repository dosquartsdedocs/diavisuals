...
Run `make check` after style changes. Run `make render-examples` only when Mermaid CLI and PlantUML are available locally. Use `make render-gallery` when the README/gallery images or compatibility profile outputs should be refreshed.

## MCP Factory Contract

This repository is a reusable, user-scoped MCP factory for diagram style assets. Keep the MCP manifest aligned with the standard ContExt lifecycle:
